...
public sealed class TodoService(ITodoRepository repository) : ITodoService {
    public async Task<IReadOnlyList<TodoItemDto>> GetAllAsync(string? filter = null,
    ...
    public async Task<TodoItemDto> CreateAsync(string title, string? description,
        CancellationToken cancellationToken = default) {
    ...
    public async Task<IReadOnlyList<TodoItemDto>> CreateRangeAsync(IEnumerable<TodoItemRequest> todoItems,
        CancellationToken cancellationToken = default) {
