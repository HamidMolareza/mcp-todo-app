...
todos.MapPut("/{id:guid}",
        async (Guid id, UpdateTodoRequest request, ITodoService service, CancellationToken cancellationToken) => {
...
todos.MapPost("/bulk",
        async (CreateTodoRangeRequest? request, ITodoService service, CancellationToken cancellationToken) => {
            ...
            if (itemsToCreate.Length == 0) {
                return Results.BadRequest(new {
                    error = "no_items_provided", message = "Provide at least one todo item with a non-empty title."
                });
            }
...
todos.MapDelete("/{id:guid}",
        async (Guid id, bool confirm, ITodoService service, CancellationToken cancellationToken) => {
            if (!confirm) {
                return Results.BadRequest(new {
